...
class Img:
    ...
    def dump(self, ix, iy, higlight=set(), f=sys.stdout):
        for y in iy:
            for x in ix:
                if (x, y) in higlight:
                    print(end="\x1b[40;31m", file=f)
                print(end=".#"[self[x, y]], file=f)
                if (x, y) in higlight:
                    print(end="\x1b[m", file=f)
            print(file=f)
        print(file=f)
...
class Svg:
    def __init__(self, fname, width, height):
        ...
        self._print(
            f'<rect width="{width*8}" height="{height*8}" style="fill:black"/>'
        )
        ...
        for y in range(img.size[1]):
            for x in range(img.size[0]):
                if (n := decode_number(img, x, y)) is not None:
                    svg.annotation(x - 0.5, y - 0.5, n[0][0] + 2, n[0][1] + 2,
                                   n[1])
        svg.close()
    elif out_fname.endswith(".txt"):
        with open(out_fname, 'w') as f:
            img.dump(range(img.size[0]), range(img.size[1]), f=f)
